...
export class ProjectComponent implements OnInit, OnDestroy {
  ...
  shareData = computed<ShareData>(() => {
    ...
    if (!project) {
      return {
        title: 'Project on Grants Platform',
        description: 'Check out this project on Grants Platform',
    ...
    let description = 'Check out this project on Grants Platform';
    if (about && about.length > 0) {
      const plainText = about.replace(/[#*_`~\[\]()]/g, '').replace(/<[^>]*>/g, '');
      description = plainText.length > 100 ? plainText.substring(0, 100) + '...' : plainText;
    } else {
      description = `${project.metadata?.name || project.metadata?.displayName || 'This project'} on Grants Platform`;
    }

    return {
      title: project.metadata?.name || project.metadata?.displayName || 'Project on Grants Platform',
